...
def apply_json_patch(game_root:str, PATH:str, patch_name:str, patch_file:str, global_patch_apply_func, IGNORE_CHECKS:bool, AJAX_VERSION:str, skip_checks:bool=False):
	...
	if not skip_checks:
		if NEEDS_GLBS and not (os.path.exists(f"{game_root}/.jxGlobals")):
			...
			if questionary.select("Install them now?", choices=["Yes", "No"]).ask() == "Yes":
				global_patch_apply_func(game_root, IGNORE_CHECKS, PATH, AJVX)
